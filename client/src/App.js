...
function App() {
  ...
  const handleTotalMarketValue = useCallback((value) => {
      setTotalMarketValue(value);
  }, []); // 依赖项数组为空，确保函数不会在每次渲染时重建

  useEffect(() => {
    document.title = "STP Server"; 
    const handleScroll = () => {
    ...
    <RefreshProvider>
     <div style={{ overflow: 'auto' }}>
      ...
      <Col  flex="none">
        ...
        <div ref={contentRef}>
          <div id="account">
            <AccountTable active={activeKey === 'account'} totalMarketValue={totalMarketValue} />
          </div>
          <div id="positions">
            <PositionsTable active={activeKey === 'positions'} onTotalValueChange={handleTotalMarketValue} />
          </div>
          <div id="orders">
